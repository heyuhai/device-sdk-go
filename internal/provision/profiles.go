...
// // LoadProfilesByMysql 从mysql读取信息
// // TODO: mysql
// func LoadProfilesByMysql(protocolType int, dic *di.Container) errors.EdgeX {
// 	deviceProfiles, err := new(dao.ModelDao).QueryModelByProtocolType(dic, protocolType)
// 	if err != nil {
// 		logger.Warn("Failed to QueryModelByProtocolType the pre-defined profiles: %v", err)
// 		return errors.NewCommonEdgeX(errors.KindServerError, fmt.Sprintf("Failed to QueryModelByProtocolType the pre-defined profiles"), err)
// 	}
//
// 	var addProfilesReq []requests.DeviceProfileRequest
// 	dpc := bootstrapContainer.DeviceProfileClientFrom(dic.Get)
